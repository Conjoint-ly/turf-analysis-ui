<template>
    <g-chart :data="data" :events="events" :options="options" :type="chartType" ref="chart"></g-chart>
</template>
...
<script>
    ...
    export default {
        name: "movements-in-preference-share-chart",
        mixins: [gchart],
        props: {
        ...
        computed: {
            minPreferenceShare() {
                return this.data.map((arr, i) =>
                    i !== 0 ? arr.map((v, i) => (i % 3 === 1 ? (v < 0 ? v : 0) : 0)).reduce((a, v) => a + v, 0) : 0
                ).reduce((a, v) => Math.min(v, a), 100);
            },
            maxPreferenceShare() {
                return this.data.map((arr, i) =>
                    i !== 0 ? arr.map((v, i) => (i % 3 === 1 ? (v > 0 ? v : 0) : 0)).reduce((a, v) => a + v, 0) : 0
