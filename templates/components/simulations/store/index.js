...
import VuexValidator from "@/plugins/vuex-validator";
...
const options = {
    ...
    plugins:
        process.env.NODE_ENV !== "production"
            ...
            : [worksheetsWatcher],
    modules: {worksheets, segments, simulations, scenarios, validations},
    actions: {
        init: function ({commit, state, dispatch}, payload) {
            commit("SET", payload);
                ...
                w.scenarios.forEach(s => {
                    s.concepts.forEach(concept => {
                        for (let i = 0; i < state.worksheets.attributes.length; i++) {
                            let attribute = state.worksheets.attributes[i];
