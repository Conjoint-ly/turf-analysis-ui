...
export default {
    ...
    mutations: {
        SET: (state, {experimentId, experimentType, csrfToken, priceObject}) => {
    ...
    actions: {
        simulationQueueIterator: ({state, commit, dispatch}) =>
            new Promise(resolve => {
        ...
        runSegregate: ({state, getters}, {worksheetId, scenarioId}) => {
            ...
            return new Promise((resolve, reject) =>
                analysis.segregate(state.experimentId, scenarioData, state.csrfToken).
                    then(data => resolve(data)).
                    catch(error => reject(error))
            );
        },
        checkDirtyAndRunSimulation: ({dispatch, getters}, worksheetId) =>
            new Promise(resolve => {
        ...
        runSourceOfBusinessSimulation: ({dispatch, getters}, worksheetId) => {
            return Promise.all(
                getters.getDirtyWorksheetIdsOfSourceOfBusiness(worksheetId).
                    map(dirtyWorksheetId => dispatch("runSimulation", dirtyWorksheetId))
            );
        },
        runSimulation: ({state, commit, dispatch, getters}, worksheetId) => {
    ...
    getters: {
        ...
        hasPriceObject: (state) => {
            return !!(state.priceObjects || state.priceObject);
        },
