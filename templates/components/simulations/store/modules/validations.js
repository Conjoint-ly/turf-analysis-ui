import attributeValidators from "@/components/simulations/validators/attribute";
import conceptNameValidators from "@/components/simulations/validators/concept-name";
import sourceOfBusinessWorksheet from "@/components/simulations/validators/source-of-business-worksheet";
import {validationPromisesReducer, withError} from "@/services/validator";
...
export default {
    actions: {
            ...
            return validationPromisesReducer(
                worksheets.map((worksheet, worksheetIndex) => {
                    if (worksheet.type === WHAT_IF)
                        return withError(dispatch)("validateWorksheet", worksheet, {worksheetIndex});
                    if (worksheet.type === SOURCE_OF_BUSINESS)
                        return withError(dispatch)("validateSourceOfBusinessWorksheet", worksheet, {worksheetIndex});
        ...
        validateSourceOfBusinessWorksheet({dispatch}, worksheet) {
            if (worksheet.type === SOURCE_OF_BUSINESS) {
                return validationPromisesReducer(
                    worksheet.scenarios.map((scenario, scenarioIndex) =>
                        withError(dispatch)("validateSourceOfBusinessScenario", {scenario}, {scenarioIndex})
                    )
                );
            }
            return Promise.resolve(true);
        ...
        validateSourceOfBusiness({dispatch}, {newWorksheet, baselineWorksheet, newScenario, baselineScenario}) {
            return validationPromisesReducer([
                withError(dispatch)("validate/some", {
                    bind: {newWorksheet, baselineWorksheet, newScenario, baselineScenario},
        ...
        validateConceptNames: function ({dispatch, getters}, {scenario, worksheetId}) {
            const map = scenario.concepts.map((c, conceptNameIndex) =>
                withError(dispatch)(
                    "validate/some",
                    {
                        bind: {
        ...
        validateAttributes({getters, dispatch}, {concept}) {
            ...
            return validationPromisesReducer(
                getters.getAttributes.map((attribute, attributeIndex) =>
                    withError(dispatch)(
                        "validate/some",
                        {
                            bind: {
        ...
        validateConceptAttributes({dispatch}, {scenario}) {
            return validationPromisesReducer(
                scenario.concepts.map((concept, conceptIndex) =>
                    withError(dispatch)("validateAttributes", {concept}, {conceptIndex})
                )
            );
        }
    }
