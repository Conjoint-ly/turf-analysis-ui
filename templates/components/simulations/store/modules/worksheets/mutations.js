...
import helpers from "@/components/simulations/store/modules/worksheets/helpers";
import {ADD_SOURCE_OF_BUSINESS_WORKSHEET, SET} from "@/components/simulations/store/mutation-types";
import format from "@/components/simulations/utils/format";
import {getBrandAttribute, getPriceAttribute} from "@/components/simulations/store/modules/worksheets/getters";
...
function fixConceptPrices(state, concept) {
    ...
    let priceLevel;
    for (let attribute in concept.attributes) {
...
export default {
    [SET]: (state, {worksheets, attributes, priceObject, segments, experimentType, applicability}) => {
        ...
        if (applicability.hasOwnProperty("version") && applicability.version === "1.0") {
            ...
            for (let brand in applicability.table) {
                        ...
                        if (experimentType === "brandPriceTradeOff") {
                            ...
                            if (
                                ...
                                brandAttribute.levels.filter(
                                    l => l.group === "npd-ours" && l.value === brand).length === 0
                            ) {
                                const aSeqNum = state.btpoAfterAdceptASeqNum;
    ...
    SET_SCENARIO_FROM_WORKSHEET_DATA: (state, {worksheet, scenarioIndex, _scenario}) => {
        ...
        _scenario.concepts.forEach((_concept, j) => {
            ...
            const actualMarketShare = concept.actualMarketShare;
            concept.actualMarketShare = actualMarketShare ? actualMarketShare : concept.simulatedPreferenceShare;
            concept.relativeAvailability = actualMarketShare ? concept.relativeAvailability : 0;
        });
        const _none = _scenario.none;
        if (_none) {
            const none = scenario.none;
            none.adjustedPreferenceShare = _none.adjustedPreferenceShare;
            none.simulatedPreferenceShare = _none.simulatedPreferenceShare;
            // applied from source concept
            const actualMarketShare = none.actualMarketShare;
            none.actualMarketShare = actualMarketShare ? actualMarketShare : none.simulatedPreferenceShare;
    ...
    SHOW_ALL_CONCEPTS: (state, worksheetId) => {
        const scenarios = state.worksheets.find(w => w.id === worksheetId).scenarios;
        scenarios.forEach(scenario => {
    ...
    SET_SCENARIO_FIELD: (state, {worksheetId, scenarioId, field, value}) => {
        Vue.set(
    ...
    SYNC_SCENARIO: (state, {scenario, newScenario}) => {
        newScenario.concepts.forEach(function (concept, i) {
    ...
    ADD_REVENUE_TO_WORKSHEET: (state, {worksheetId, revenue: {title, xAxisTitle, formulas}}) => {
        ...
        const worksheet = state.worksheets.find(w => w.id === worksheetId);
        Vue.set(worksheet.revenues, id, {id, title, xAxisTitle, formulas});
        Vue.set(state.currentRevenueIds, worksheetId, id);
    },
    CHANGE_REVENUE: (state, {worksheetId, revenueId, title, xAxisTitle, formulas}) => {
        const worksheet = state.worksheets.find(w => w.id === worksheetId);
        Vue.set(worksheet.revenues, revenueId, {id: revenueId, title, xAxisTitle, formulas});
    },
