...
import generate from "@/components/simulations/utils/generate";
import unionBy from "lodash/unionBy";
import pick from "lodash/pick";
import format from "@/components/simulations/utils/format";
...
export const getters = {
    ...
    getCurrentScenarioIndex: state =>
        state.currentScenarioIndexes[state.currentWorksheetIndex] || state.defaultCurrentScenarioIndex,
    ...
    getScenarioField: (state, getters) => (worksheetId, scenarioId, field) => {
        return getters.getScenarioById({worksheetId, scenarioId})[field];
    },
    ...
    getApplicabilityLevels: (state, getters) => (concept, attribute) =>
        attribute.levels.filter(level => getters.getShownLevels(concept, attribute).indexOf(level.value) !== -1),
    ...
    getMovementsInPreferenceShareChartData: () => worksheet => {
        ...
        scenarios.forEach(scenario => {
            const cells = [];
            const newScenario = scenario.newScenario;
            const baselineScenario = scenario.baselineScenario;
            ...
            concepts.forEach(concept => {
                ...
                const baselineConcept =
                    baselineScenario && baselineScenario.concepts.find(c => c.name === concept.name);
                const move =
                    (newConcept ? newConcept.adjustedPreferenceShare : 0.0) -
            ...
            nones.forEach(none => {
                const newNone =
                    newScenario && newScenario.none && newScenario.none.name === none.name ? newScenario.none : null;
                const baselineNone =
                    baselineScenario && baselineScenario.none && baselineScenario.none.name === none.name
                        ? baselineScenario.none
                        : null;
                const move =
                    (newNone ? newNone.adjustedPreferenceShare : 0.0) -
    ...
    getChartData: (state, getters) => ({worksheetId, field}) => {
        ...
        const scenarios = getters.getWorksheetById(worksheetId).scenarios;
    ...
    getRevenue: (state, getters) => (worksheetId, revenueId) => {
        const worksheetById = getters.getWorksheetById(worksheetId);
        const concepts = worksheetById.scenarios[0].concepts;
        let formulas = {};
        concepts.map(c => (formulas[c.name] = state.default.defaultFormula));
    ...
    getRevenues: function (state, getters) {
        return worksheetId => {
            const worksheet = getters.getWorksheetById(worksheetId);
    ...
    getWorksheetElasticityChartVariableConcept: state => worksheetId =>
        state.worksheetsElasticityChartVariableConcept[worksheetId] || [],
    getForExportSourceOfBusinessToCsv: (state, getters) => worksheetId => {
        ...
        scenarios.forEach(scenario => {
            ...
            nones.forEach(none => {
                ...
                const baselineNone =
                    baselineScenario.none && baselineScenario.none.name === none.name ? baselineScenario.none : null;
                const move =
                    (newNone ? newNone.adjustedPreferenceShare : 0.0) -
                    (baselineNone ? baselineNone.adjustedPreferenceShare : 0.0);
    ...
    getShownLevels: (state, getters) => (concept, attribute) => {
        ...
        hiddenLevels = hiddenLevels ? hiddenLevels : [];
