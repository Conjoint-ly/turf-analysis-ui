...
import {SOURCE_OF_BUSINESS} from "@/components/simulations/worksheet-types";
import cloneDeep from "lodash/cloneDeep";
import omit from "lodash/omit";
import $ from "jquery";
import helpers from "@/components/simulations/store/modules/worksheets/helpers";
import {ADD_SOURCE_OF_BUSINESS_WORKSHEET} from "@/components/simulations/store/mutation-types";
...
export function saveWorksheets(
    ...
    return new Promise((resolve, reject) => {
        ...
        dispatch("validateSimulation", worksheets).then(valid => {
            ...
            dispatch("selectWorksheet", errorObject.worksheetIndex).then(() =>
                dispatch("selectScenario", errorObject.scenarioIndex).then(() => {
                    window.newNotifier({
...
export function copyWithRotating({commit, getters}, {worksheetId, scenario, conceptCountToCopy}) {
    const count =
        conceptCountToCopy > scenario.concepts.length
        ...
        if (
            ...
            dispatch("copyWorksheet", {worksheetId: worksheet.id}).then(newWorksheet => {
                commit("SET_WORKSHEET_NAME", {
                    ...
                    name: `Sensitivity analysis to ${attribute.name} in ${scenario.concepts[conceptIndex].name}`
                });
            ...
            resolve({worksheet: worksheet, scenarioId});
        }
...
export function setScenarioField({commit}, {worksheetId, scenarioId, field, value}) {
    return new Promise(resolve => {
        commit("SET_SCENARIO_FIELD", {worksheetId, scenarioId, field, value});
        resolve();
    });
}
...
export function checkApplicability({rootState, state, commit, getters}, {concept, attribute, value}) {
