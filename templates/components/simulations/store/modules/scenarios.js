...
const extractors = {
    getShowRelativeAvailable: scenario =>
        scenario.concepts && scenario.concepts.filter(
            concept => concept.relativeAvailability != 0 && concept.relativeAvailability != 1).length > 0 ||
        (scenario.none && scenario.none.relativeAvailability != 0 && scenario.none.relativeAvailability != 1),

    getShowAdvancedSettings: (scenario, showRelativeAvailable) =>
        scenario.productType !== "sop" || showRelativeAvailable || scenario.segment != 0
};
...
export default {
    ...
    mutations: {
        [SET]: (state, {worksheets}) => {
            worksheets.filter(w => w.type === WHAT_IF).map(worksheet =>
                worksheet.scenarios.map(scenario => {
                    setter(state)(scenario);
                })
            );
            worksheets.filter(w => w.type === SOURCE_OF_BUSINESS).map(worksheet =>
                worksheet.scenarios.map(scenario => {
        ...
        SET_SHOW_ADVANCED_SETTINGS_FOR_ALL_SCENARIOS: (state, value) => {
            for (let prop in state.scenariosShowAdvancedSettings) {
    ...
    getters: {
        ...
        getShowAdvancedSettings: (state, getters) => scenario =>
            state.scenariosShowAdvancedSettings[scenario.id] ||
