<template functional>
	...
	>
		<label v-if="props.label" :class="['control-label', props.labelClass ? props.labelClass : 'col-xs-4 col-sm-3 col-lg-2']">{{ props.label }}</label>
...
export default {
	...
	hasError(validation, field) {
		if (Array.isArray(field)) {
			return field.reduce(
				(result, item) =>
					(result = result || validation.hasError(item)),
				false
			);
		}
	...
	isPassed(validation, field) {
		if (Array.isArray(field)) {
			return field.reduce(
				(result, item) =>
					(result = result && validation.isPassed(item)),
				true
			);
		}
