			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">
						...
						<component v-else :is="header.component" v-bind="header.props" v-on="header.listeners"></component>
					</h4>
				</div>

				<div class="modal-body">
					<component :is="body.component" v-bind="body.props" v-on="body.listeners"></component>
				</div>

				<div v-if="footer.visible" class="modal-footer">
                    <template v-if="!footer.component">
                        <button type="button" class="btn btn-default" @click="cancel">{{ footer.cancel }}</button>

                        <button v-if="isLoading" type="button" class="btn btn-success" @click="save" disabled>
                            <i class="far fa-spinner fa-spin"></i>
                            {{ footer.loading }}
                        </button>
                        <button v-else type="button" class="btn btn-success" @click="save">{{ footer.save }}</button>
                    </template>
                    <component v-esle :is="footer.component" v-bind="footer.props" v-on="footer.listeners"></component>
				</div>
			</div>
		</div>
		<div class="modal-backdrop fade" :class="[isVisible]" @click="shake"></div>
	</div>
</template>
